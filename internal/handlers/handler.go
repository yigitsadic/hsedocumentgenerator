...
const (
	...
	pdfGeneratedText                   = "👍\t[%s - %s.pdf]\t%s\tiçin PDF belgesi üretildi.\n"
	...
	errorOccurredDuringPDFCreationText = "😥\t[%s - %s.pdf] %s için beklenmedik bir hata oluştu.\n"
...
func (h *Handler) GeneratePDF(r models.Record) error {
	...
	h.Files = append(h.Files, models.ReadFile{FileName: fmt.Sprintf("%s - %s.pdf", r.UniqueReference, r.FullName), Content: result})

	h.Write(pdfGeneratedText, r.UniqueReference, r.FullName, r.FullName)
	return nil
}
...
func (h *Handler) Do() {
	...
	for _, record := range h.ReadRecords {
		...
		if err != nil {
			h.Write(errorOccurredDuringPDFCreationText, record.UniqueReference, record.FullName, record.FullName)
		}
	}
